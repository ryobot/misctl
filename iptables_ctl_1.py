...
req = {
    ...
    'rule_id':"", 'target':"", 'prot':"", 'opt':"", 'ifin':"", 'ifout':"", 'source':"", 'sport':"", 'destination':"", 'dport':"", 'misc':""
}
for key in req.keys():
    if form.has_key(key):
...
params = { 
    ...
    'sport':req['sport'], 'destination':req['destination'], 'dport':req['dport'], 'misc':req['misc']
}
...
if service.state == "running":
    ...
    for rule in stats.forward_chain.rules:
        ...
        params += ', "' + rule.source + d + rule.sport + d + rule.destination + d + rule.dport + d +rule.misc + '"'
